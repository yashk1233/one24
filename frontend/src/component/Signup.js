...
export class Signup extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    try {
      ...
      const { name, email, psd, cpsd } = this.state;
      console.log(name, email, psd, cpsd);
      ...
      if (psd == cpsd) {
        ...
        const response = await axios.post(signupUrl, JSON.stringify({ name: name, uemail: email, upassword: psd }), {
          crossDomain: true,
          headers: { 'Content-Type': 'application/json' },
          withCredentials: true
        });
        const myres = JSON.stringify(response?.data);

        if (myres == '{"status":"User Exist"}') {
        ...
        else if(myres == '{"status":"ok"}'){

          document.getElementById("userexist").style.color="green";
          document.getElementById("userexist").innerHTML="Successfully created Account ";
        }
        ...
        document.getElementById("userexist").style.display = "flex";
        document.getElementById("userexist").style.color = "red";
        document.getElementById("userexist").innerHTML = "Password Does'nt match";
              ...
              <svg className="h-6 w-6 text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 10.5v3.75m-9.303 3.376C1.83 19.126 2.914 21 4.645 21h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 4.88c-.866-1.501-3.032-1.501-3.898 0L2.697 17.626zM12 17.25h.007v.008H12v-.008z" />
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      ...
                      <input placeholder='Name' className="footer" type="text" onChange={(e) => this.setState({ name: e.target.value })} />
                      {/* <h1>Enter Name</h1> */}
                      ...
                      <input required placeholder='Confirm Password' className="footer" type="password" onChange={(e) => this.setState({ cpsd: e.target.value })} />
